...
describe('ConfigParser', () => {
  describe('parseConfig', () => {
    it('should successfully parse valid configuration', () => {
      const validConfig = {
        ...
        expectedDomainAndMessageHashes: {
          ...
          messageHash: '0x88aac3dc27cc1618ec43a87b3df21482acd24d172027ba3fbb5a5e625d895a0b',
        },
        stateOverrides: [
          {
            name: 'Test Contract',
      ...
      expect(result.config.taskName).toBe('test-task');
    });
    ...
    it('should successfully parse real-world base-nested.json configuration', () => {
      ...
      expect(result.config.taskName).toBe('mainnet-upgrade-system-config');

      // Validate expected hashes
      expect(result.config.expectedDomainAndMessageHashes.address).toBe(
        '0x9C4a57Feb77e294Fd7BF5EBE9AB01CAA0a90A110'
      );
      expect(result.config.expectedDomainAndMessageHashes.domainHash).toBe(
        '0x88aac3dc27cc1618ec43a87b3df21482acd24d172027ba3fbb5a5e625d895a0b'
      );
      expect(result.config.expectedDomainAndMessageHashes.messageHash).toBe(
        '0x9ef8cce91c002602265fd0d330b1295dc002966e87cd9dc90e2a76efef2517dc'
      );

      // Validate state overrides
      expect(result.config.stateOverrides).toHaveLength(3);
      expect(result.config.stateOverrides[0].name).toBe('ProxyAdminOwner');
      expect(result.config.stateOverrides[1].name).toBe('Base Multisig');
      expect(result.config.stateOverrides[2].name).toBe('Base Nested Multisig');

      // Validate Base Nested Multisig has 2 overrides
      expect(result.config.stateOverrides[2].overrides).toHaveLength(2);

      // Validate state changes
      expect(result.config.stateChanges).toHaveLength(4);
      expect(result.config.stateChanges[0].name).toBe('System Config');
      expect(result.config.stateChanges[1].name).toBe('Proxy Admin Owner');
      expect(result.config.stateChanges[2].name).toBe('Base Multisig');
      expect(result.config.stateChanges[3].name).toBe('Base Nested Multisig');

      // Validate Proxy Admin Owner has 2 changes
      expect(result.config.stateChanges[1].changes).toHaveLength(2);

      // Validate Base Multisig has 2 changes
      expect(result.config.stateChanges[2].changes).toHaveLength(2);
    });
    ...
    it('should handle invalid data types', () => {
      const invalidTypeConfig = {
        ...
        expectedDomainAndMessageHashes: {
          ...
          domainHash: 'not_a_hash',
          messageHash: 'not_a_hash',
        },
        stateOverrides: [],
        stateChanges: [],
      };
    ...
    it('should handle invalid formats (addresses and hashes)', () => {
      const invalidFormatConfig = {
        ...
        expectedDomainAndMessageHashes: {
          ...
          domainHash: 'invalid_hash_format', // Invalid: not 32-byte hex
          messageHash: '0x88aac3dc27cc1618ec43a87b3df21482acd24d172027ba3fbb5a5e625d895a0b', // Valid hash
        },
        stateOverrides: [
          {
            name: 'Test Contract',
      ...
      expect(errorPaths).toContain('expectedDomainAndMessageHashes.domainHash');
      expect(errorPaths).toContain('stateOverrides.0.address');
      expect(errorPaths).toContain('stateOverrides.0.overrides.0.value');
      expect(errorPaths).toContain('stateChanges.0.changes.0.key');
    });
  });
  ...
  describe('parseFromString', () => {
    ...
    it('should handle invalid JSON string', () => {
      const invalidJson = '{ "taskName": "test", invalid }';
  ...
  describe('getValidationSummary', () => {
    ...
    it('should return detailed error summary for invalid config', () => {
      const invalidConfig = { taskName: 123 };
      ...
      expect(summary).toContain('taskName: Expected string, received number');
    });
