import { DiffComparator } from '../../src/lib/comparator';
import { StateChange, StateOverride } from '../../src/lib/types/index';
...
describe('DiffComparator', () => {
  ...
  describe('compareStateOverride', () => {
    ...
    it('should handle extra overrides', () => {
      ...
      expect(extraKeyField!.actual).toBe(
    ...
    it('should handle missing overrides', () => {
      ...
      expect(missingKeyField!.expected).toBe(
    ...
    it('should handle multiple differences in one comparison', () => {
      ...
      expect(result.status).toBe('mismatch');
      expect(result.summary).toContain('❌ StateOverride differences found:');
      expect(result.stats.mismatchedFields).toBeGreaterThanOrEqual(4); // name, address, key, value

      // Check that all modified fields are detected
      const fieldPaths = result.diffs[0].fieldDiffs
  ...
  describe('edge cases', () => {
    it('should handle complex nested differences', () => {
      ...
      expect(mismatchedFields.length).toBeGreaterThanOrEqual(2); // key, value
    });
  });
